...
generate_coords <- function(curriculum_graph) {
  ...
  #
  num_in_section <- array(0,dim = c(4,8))
  y_coord <- 0
  old_dept <- "not!"
  old_term <- 0
  for (i in curriculum_graph$node_list$id) {
    ...
    num_in_section[y_starter,term]<-section_depth+1
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "dashboard",
        fluidRow(
          box(visNetworkOutput("mynetworkid", height = 500,width=500))
          )
        )
      ),
